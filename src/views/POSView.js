...
const POSView = ({ products, cart, addToCart, updateQuantity, total, onInitiateCheckout, syncStatus, searchTerm, setSearchTerm }) => {
  ...
  return (
    <>
      ...
      <div className="w-3/5 bg-white rounded-lg shadow p-4 flex flex-col">
        ...
        {/* --- PERUBAHAN DI SINI --- */}
        {/* Jika 'products' belum terdefinisi (sedang loading), tampilkan skeleton */}
        {products === undefined ? (
          <ProductSkeleton />
        ) : (
      ...
      <div className="w-2/5 bg-white rounded-lg shadow p-4 flex flex-col">
        ...
        <div className="border-t pt-4">
          ...
          <button onClick={onInitiateCheckout} disabled={cart.length === 0} className="w-full bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 disabled:bg-gray-400 transition-colors">
            Bayar
          </button>
